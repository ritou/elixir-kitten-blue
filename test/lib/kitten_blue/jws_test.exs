defmodule KittenBlue.JWSTest do
  ...
  @hs256_jwk_1 [
                 ...
                 alg: "HS256",
                 key:
                   "yx1dSdNR624-DZF1HdpdHeRb2Pa2AGaqF-ig8HCsPwU"
  ...
  @hs256_jwk_2 [
                 ...
                 alg: "HS256",
                 key:
                   "XrjkdL7Ma6xxNmiIj0_K1UkztTakF0Dd6k1fyYjd-kI"
                   |> Base.url_decode64!(padding: false)
                   |> JOSE.JWK.from_oct()
               ]
               |> JWK.new()
  ...
  describe "verify error" do
    ...
    test ":invalid_jwt_header" do
      header = %{"typ" => "secevent+jwt"}
      payload = %{"foo" => "var"}
      assert {:ok, jws} = JWS.sign(payload, @rs256_jwk_1, header)

      assert {:error, :invalid_jwt_header} ==
               JWS.verify(jws, [@rs256_jwk_1, @rs256_jwk_2], %{"typ" => "jwt"})
    end
  end
end
